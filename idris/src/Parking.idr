...
||| Another version of parking price that works backwards and thus is
||| guaranteed to terminate (e.g. is primitive recursive)
data ParkingPrice  : (price : Nat) -> (duration: Nat) -> Type where

  ||| More than one day (possibly exactly one day)
  MoreThanADay : (fulldays : Nat) -> (fraction : ParkingPrice pr dur) ->
                 ParkingPrice (fulldays * 170) (1440 * fulldays + dur)

  MoreThan11h :  { auto prf : LT dur duration}
              -> (fraction : ParkingPrice pr dur)
              -> ParkingPrice pr duration

  MoreThan10h45 : { auto prf : LT dur duration}
                -> (fraction : ParkingPrice pr dur)
                -> ParkingPrice (pr + 1) duration

  MoreThan1h30 : { auto prf : LT dur duration}
               -> (fraction : ParkingPrice pr dur)
               -> ParkingPrice (pr + (((-) duration dur {smaller = lteSuccLeft prf}) `div` 15 * 4)) duration

  More15Mins : { auto prf : LT dur duration}
             -> (fraction : ParkingPrice pr dur)
             -> ParkingPrice (pr + (((-) duration dur {smaller= lteSuccLeft prf}) `div` 15 * 3)) duration

  MoreThan10Mins : { auto prf : LT dur duration}
                 -> (fraction : ParkingPrice pr dur)
                 -> ParkingPrice 6 duration

  LessThan10Mins : { auto prf : LTE duration 10}
                 -> ParkingPrice 0 duration
