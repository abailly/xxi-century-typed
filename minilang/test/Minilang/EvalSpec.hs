...
spec :: Spec
spec = parallel $
    describe "Expressions Evaluator" $ do
        ...
        prop "evaluates universes" $ \(Universe u@(U n)) -> eval u emptyEnv === EU n
        ...
        it "evaluates product type" $ do
            eval (Pi (B "foo") (U 0) (Var "bar")) emptyEnv
                `shouldBe` EPi (EU 0) (Cl (B "foo") (Var "bar") emptyEnv)

        it "evaluates sum type" $ do
            eval (Sigma (B "foo") (U 0) (Var "bar")) emptyEnv
                `shouldBe` ESig (EU 0) (Cl (B "foo") (Var "bar") emptyEnv)

        it "evaluates projections" $ do
            let extended =
                    ExtendPat
                        (ExtendPat emptyEnv (B "x") (ENeut $ NV $ NVar 1))
                        (B "y")
                        (ENeut $ NV $ NVar 2)
        ...
        it "evaluates Sum definition" $ do
            ...
            eval
                (Sum [Choice "true" (Just Unit), Choice "false" Nothing])
                extended
                `shouldBe` ESum
                    ( SumClos
                ...
                ( Let
                    ( RDecl
                        ...
                        ( Abs
                            (B "x")
                            ( Case
                                [ Clause "zero" (Abs Wildcard (Var "x"))
                                , Clause
                                    "succ"
                                    ( Abs
                                        (B "y1")
...
newtype Universe = Universe AST
    deriving (Show, Eq)

instance Arbitrary Universe where
    arbitrary = Universe . U . fromInteger . getPositive <$> arbitrary
