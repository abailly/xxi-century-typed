...
fakeLogger = LoggerEnv Nothing "foo" (const $ pure ()) (const $ pure ()) (const id) (pure ())
...
newLog loggerId = liftIO $ do
  ...
  return $
    LoggerEnv
      { logger = Just inchan,
        logInfo = logEvent' inchan loggerId,
        logError = logError' inchan loggerId,
        withLog = withLog' inchan loggerId,
